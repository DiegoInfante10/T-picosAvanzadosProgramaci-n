...
public final class Registro extends JFrame{
    ...
    public void initComponents(){
        ...
        check();
        jtextFields.add(nombreT);  jtextFields.add(edadT);  jtextFields.add(pesoT);  jtextFields.add(alturaT); 
    ...
    public void check(){
        if(listado.isEmpty()) { eliminar.setEnabled(false); buscar.setEnabled(false); } else { eliminar.setEnabled(true); buscar.setEnabled(true); }
    }
    
    public boolean JTextIsEmpty(){
    ...
    public void refrescarDatos(int i) {
        nombreT.setText(listado.get(i).getNombre()); edadT.setText(String.valueOf(listado.get(i).getEdad()));
        pesoT.setText(String.valueOf(listado.get(i).getPeso())); alturaT.setText(String.valueOf(listado.get(i).getAltura()));
        IMCv.setText(String.valueOf(listado.get(i).getIMC())); 
    }
    
    public void checkNandP(){
        if(listado.size() <= 1){
    ...
    public boolean in(String persona){
        boolean centinela = false;
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                if(ae.getSource() == ant){
                    ...
                    if(!listado.isEmpty()) {
                        ...
                        JOptionPane.showMessageDialog(null, "Personas vaciadas al archivo");
                    } else {
                    ...
                    if(!listado.isEmpty()){
                        temp = new EscritorArchivo().cargarDatos(nombreArch);
                        for (Persona p : temp) {
                            listado.add(p);
                        }
                    } else {
                        listado = new EscritorArchivo().cargarDatos(nombreArch);
                    }
                    index = 0;
                    contador = index + 1;
                    contadorL.setText("Persona " + contador +" de " + listado.size());
